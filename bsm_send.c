...
int main (int argc, char **argv) {
	...
	//WSSP header
	struct wsshdr wsspHeader;
	// WSS packet container
	byte_t *wsspacket;

	//WSMP header
	struct wsmphdr wsmpHeader;
	// WSM packet container
	byte_t *wsppacket;
	...
	ret_wlanl_val=wlanLookup(devname,&ifindex,srcmacaddr,NULL,0,WLANLOOKUP_NONWLAN);
	if(ret_wlanl_val<=0) {
	...
	etherheadPopulateB(&etherHeader, srcmacaddr, ETHERTYPE_WSMP);
	...
	// WSSPheadPopulate(&wsspHeader,0x3,0);

	//WSMPheadPopulate
	WSMPheadPopulate(&wsmpHeader,0x2,0xC00305);


	// Allocating packet buffers
	wsppacket=malloc(WMSP_PACKET_SIZE(V2X_MSG_BUF));
	// wsspacket=malloc(WSSP_PACKET_SIZE(V2X_MSG_BUF));

	// udppacket=malloc(UDP_PACKET_SIZE(V2X_MSG_BUF));
	// ippacket=malloc(IP_UDP_PACKET_SIZE(V2X_MSG_BUF));
	ethernetpacket=malloc(ETH_IP_WSP_PACKET_SIZE(V2X_MSG_BUF));
	...
	while(1) {
		...
		if(poll(&timerMon,1,INDEFINITE_BLOCK)>0) {
			...
			// WSSPencapsulate(wsspacket, &wsspHeader,V2X_MSG_BUF,WSSP_PACKET_SIZE(V2X_MSG_BUF));

			WSMPencapsulate(wsppacket, &wsmpHeader,V2X_MSG_BUF,sizeof(V2X_MSG_BUF));

			finalpktsize=etherEncapsulate(ethernetpacket, &etherHeader, wsppacket, ETH_IP_WSP_PACKET_SIZE(V2X_MSG_BUF));
			...
			id+=INCR_ID;
			UDPencapsulate(udppacket,&udpHeader,wsppacket,WMSP_PACKET_SIZE(V2X_MSG_BUF),ipaddrs);
			...
			IP4Encapsulate(ippacket, &ipHeader, udppacket, UDP_PACKET_SIZE(V2X_MSG_BUF));
			finalpktsize=etherEncapsulate(ethernetpacket, &etherHeader, ippacket, IP_UDP_PACKET_SIZE(V2X_MSG_BUF));
#endif
			// Send datagram
